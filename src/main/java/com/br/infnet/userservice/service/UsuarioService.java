...
public class UsuarioService {
    ...
    public UsuarioResponse getUsuarioInfoById(UUID id) {
        log.info("Consultando dados de usuário para pagamento");
        Usuario usuario = usuarioRepository.findById(id)
                .orElseThrow(() -> new UsuarioNotFoundException("Usuário não encontrado com o ID: " + id));
        if (usuario.getStatus() == Status.INATIVO) {
            throw new UsuarioNotFoundException("Usuário não encontrado com o ID: " + id);
        }
        log.info("Dados para pagamento encontrados para usuário de ID: {}", id);
    ...
    public VendedorResponseInfo getVendedorInfoById(UUID id) {
        ...
        log.info("Consultando informações de vendedor para usuário ID: {}", id);
    ...
    public void deletarUsuario(UUID id) {
        log.info("Iniciando deleção de usuário: {}", id);
        Usuario usuario = usuarioRepository.findById(id)
                .orElseThrow(() -> new UsuarioNotFoundException("Usuário não encontrado com o ID: " + id));
        if (usuario.getStatus() == Status.INATIVO) {
            return;
    ...
    public void alterarFotoDePerfil(String urlNovaFoto, UUID userId) {
        log.info("Iniciando alteração de foto de perfil para usuário: {}", userId);
        ...
        log.info("Foto de perfil alterada para usuário: {}", userId);
    }
